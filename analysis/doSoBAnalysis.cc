...
int main(int argc, char** argv)
{
  ...
  int numSteps=30;
	...
	cout <<"testing cut " << pCut <<endl;
        sprintf(buffer,"%s tagCorr*(recDecaySignature  &&  mDnPi> %f  &&mNu2<%f && mNu2>%f && numRecPions==%d && mBTag> 5.27 && deltaETag>-0.05 && deltaETag<0.05 && logProb > -3 && mDnPi < 3.0 && bestBCharge==((-1)*systemCharge))  ",corrBuffer,pCut,upperCut,lowerCut,numPions);
    ...
    cout <<"x axis " << endl;
    grFoM.GetXaxis()->SetTitle("min m Dnpi momentum[GeV]");
    cout <<"drawing.." <<endl;
    grFoM.Draw("AP*");
    c.SaveAs("fom.png");

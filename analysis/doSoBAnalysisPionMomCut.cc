...
int main(int argc, char** argv)
{
  ...
  int numSteps=30;
	...
	float pCut=upperPCut+i*pCutStep;
		...
		if(numPions==2)
		  {
		    sprintf(buffer,"%s tagCorr*(recDecaySignature && pi2Mom<%f &&mNu2<%f && mNu2>%f && numRecPions==%d && mBTag> 5.27 && deltaETag>-0.18 && deltaETag<0.18 && logProb > -3  && bestBCharge==((-1)*systemCharge))  ",corrBuffer,pCut,upperCut,lowerCut,numPions);
		  }
		else
		  {
    ...
    if(numPions==1)
      ...
      c.SaveAs("fomTwoPionUpperPionCutNoDnCut.png");

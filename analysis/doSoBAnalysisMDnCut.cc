...
int main(int argc, char** argv)
{
  ...
  int numSteps=40;
    ...
    for(int i=0; i<numSteps;i++)
      {
	float mDnCut=upperPCut+i*pCutStep;
	cout <<"testing cut " << mDnCut <<endl;
	sprintf(buffer,"%s tagCorr*(recDecaySignature &&mNu2<%f && mNu2>%f && numRecPions==%d && mBTag> 5.27 && deltaETag>-0.05 && deltaETag<0.05 && logProb > -3 && mDnPi < %f && bestBCharge==((-1)*systemCharge))  ",corrBuffer,upperCut,lowerCut,numPions,mDnCut);
    ...
    cout <<"x axis " << endl;
    grFoM.GetXaxis()->SetTitle("max M_{Dn#pi} [GeV]");
    cout <<"drawing.." <<endl;
    grFoM.Draw("AP*");
    if(numPions==1)
      c.SaveAs("fomMDnPi1.png");
    else
      c.SaveAs("fomMDnPi2.png");

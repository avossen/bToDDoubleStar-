...
int main(int argc, char** argv)
{
  ...
  //  for(int iC=0;iC<2;iC++)
      ...
      if(numPions==2)
	{
      ...
      float upperPCut=5.10;
      //      float pCutStep=0.007;
      float pCutStep=0.01;
      int numSteps=30;
      ...
      for(int i=0; i<numSteps;i++)
	{
	  ...
	  for(int iT=0;iT<6;iT++)
		...
		huschleLumiFactor=chargedFactor;
	      ...
	      sprintf(buffer,"%s (1+foundAnyDDoubleStar*%f)*tagCorr*(recDecaySignature  &&mNu2<%f && mNu2>%f && numRecPions==%d && mBTag> %f && deltaETag>(-1)*0.18 && deltaETag<0.18 && logProb > -3.5 && mDnPi < 3.0 && bestBCharge==((-1)*systemCharge))  ",corrBuffer,huschleLumiFactor,upperCut,lowerCut,numPions,upperDeltaM);
    ...
    cout <<"x axis " << endl;
    grFoM.GetXaxis()->SetTitle(" #Delta M Cut[GeV]");
    cout <<"drawing.." <<endl;
    grFoM.Draw("AP*");
    sprintf(buffer,"fomDeltaMCut_%d_%s.png",numPions,channelString);
    c.SaveAs(buffer);

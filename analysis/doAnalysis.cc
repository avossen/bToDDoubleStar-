...
int main(int argc, char** argv)
{
  ...
  glColorTable[9]=gROOT->GetColor(kWhite);


  TH1F* components[10*4];
  //the allocation of the actual histograms is done in the functions
  TH1F* summedComponents[3][2][10];
  ...
  for(vector<int>::iterator it=channelsUnderConsideration.begin();it!=channelsUnderConsideration.end();it++)
         ...
         for(int i=0;i<1;i++)      
	{
	  ...
	  if(!loadFromFile)
	    {
	      ...
	      getMCComponents(trees,components,summedComponents[i][pionIndex], numPions,leptonId,iC);
	    }
	  else
	    {
	      loadComponents(mF,components,summedComponents[i][pionIndex],numPions,leptonId,iC,gl_numComponents,gl_numFiles);
	    }
	  ...
	  saveStack(components,summedComponents[i][pionIndex],numPions,leptonId,iC);
	  //      for(
...
#ifdef USE_DATA
	  fitFractions(trees,summedComponents[i][pionIndex],10, numPions,leptonId,iC,true,false,pulls);
#else
#ifdef PARTIAL_BOX
	  fitFractions(trees,summedComponents[i][pionIndex],10, numPions,leptonId,iC,true,false,pulls);
#else
	  ...
	  fitFractions(trees,summedComponents[i][pionIndex],10, numPions,leptonId,iC,true,false,pulls);
#else
	  //  fitFractions(trees,summedComponents[i][pionIndex],10, numPions,leptonId,iC,false,true,pulls);
  fitFractions(trees,summedComponents[i][pionIndex],10, numPions,leptonId,iC,false,true,pulls);
#endif
#endif
#endif

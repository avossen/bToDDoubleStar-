...
int main(int argc, char** argv)
{
  ...
  //  for(int iC=0;iC<2;iC++)
      ...
      for(int i=0; i<numSteps;i++)
	{
	  ...
	  for(int iT=0;iT<6;iT++)
		...
		huschleLumiFactor=chargedFactor;
	      ...
	      sprintf(buffer,"%s (1+foundAnyDDoubleStar*(%f))*tagCorr*(recDecaySignature  &&mNu2<%f && mNu2>%f && numRecPions==%d && mBTag> 5.27 && deltaETag>(-1)*%f && deltaETag<%f && logProb > -3 && mDnPi < 4.0 && bestBCharge==((-1)*systemCharge))  ",corrBuffer,huschleLumiFactor,upperCut,lowerCut,numPions,upperDeltaE,upperDeltaE);
    ...
    cout <<"x axis " << endl;
    grFoM.GetXaxis()->SetTitle(" #Delta E Cut[GeV]");
    cout <<"drawing.." <<endl;
    grFoM.Draw("AP*");
    sprintf(buffer,"fomDeltaECut_%d_%s.png",numPions,channelString);
    c.SaveAs(buffer);

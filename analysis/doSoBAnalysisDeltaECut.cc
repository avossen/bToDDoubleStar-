...
int main(int argc, char** argv)
{
  ...
  int numSteps=30;
	...
	float upperDeltaE=pCut;
		...
		sprintf(buffer,"%s tagCorr*(recDecaySignature  &&mNu2<%f && mNu2>%f && numRecPions==%d && mBTag> 5.27 && deltaETag>(-1)*%f && deltaETag<%f && logProb > -3 && mDnPi < 3.0 && bestBCharge==((-1)*systemCharge))  ",corrBuffer,upperCut,lowerCut,numPions,upperDeltaE,upperDeltaE);
    ...
    grFoM.Draw("AP*");
    if(numPions==1)
      c.SaveAs("fomDeltaECut1.png");
    else
      c.SaveAs("fomDeltaECut2.png");
}
